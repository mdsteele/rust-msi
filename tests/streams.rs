...
fn invalid_stream_name() {
    ...
    assert_error!(package.read_stream("\u{4840}Foo"),
                  ErrorKind::InvalidInput,
                  "\"\u{4840}Foo\" is not a valid stream name");
    assert_error!(package.write_stream("\u{4840}Bar"),
                  ErrorKind::InvalidInput,
                  "\"\u{4840}Bar\" is not a valid stream name");
    assert_error!(package.remove_stream("\u{4840}Baz"),
                  ErrorKind::InvalidInput,
...
fn nonexistent_stream_name() {
    let cursor = Cursor::new(Vec::new());
    let mut package = Package::create(PackageType::Installer, cursor).unwrap();
    assert_error!(package.read_stream("Foo"),
                  ErrorKind::NotFound,
                  "Stream \"Foo\" does not exist");
    assert_error!(package.remove_stream("Baz"),
                  ErrorKind::NotFound,
